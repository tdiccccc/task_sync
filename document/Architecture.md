...
- コンポーネントは**Atomic Design**の考え方に基づき、粒度別に分類する
- Atomic Designの全5階層のうち、Templates と Pages は Nuxt 3 の `layouts/` と `pages/` が担うため、`components/` 配下では **Atoms / Molecules / Organisms** の3階層を使用する

#### Atomic Design レイヤー定義

| レイヤー | ディレクトリ | 粒度 | 例 |
|---------|-------------|------|-----|
| **Atoms** | `app/components/atoms/` | 最小単位のUI部品。単一のHTML要素またはReka UIプリミティブのラッパー | Button, Input, Label, Badge, Icon |
| **Molecules** | `app/components/molecules/` | 複数のAtomsを組み合わせた小さな機能単位 | FormField（Label + Input + ErrorMessage）, SearchBox, NavLink |
| **Organisms** | `app/components/organisms/` | Molecules/Atomsを組み合わせた独立したUIセクション | Sidebar, Header, LoginForm, DashboardCardGrid |
| **Layouts** | `app/layouts/` | ページ共通の枠組み（Nuxt 3 レイアウト = Atomic DesignのTemplates相当） | default.vue, auth.vue |
| **Pages** | `app/pages/` | ルーティング対象の画面（Nuxt 3 ページ = Atomic DesignのPages相当） | index.vue, login.vue |

#### 配置ルール

- **Atoms**: 他のコンポーネントに依存しない。Props と Emits のみで外部とやり取りする
- **Molecules**: Atoms のみをインポートできる。composables の利用は可
- **Organisms**: Atoms, Molecules をインポートできる。composables やストアへの依存も可
- **命名規則**: 各レイヤーのプレフィックスは不要（ディレクトリで区別する）。ファイル名はPascalCase（例: `FormField.vue`）
...
│   │   ├── FormField.vue
│   │   └── NavLink.vue
│   └── organisms/       # 独立したUIセクション
│       ├── Sidebar.vue
│       └── LoginForm.vue
├── composables/         # ロジックの再利用（状態管理・API通信）
├── layouts/             # レイアウト（= Atomic Design の Templates）
├── pages/               # ページコンポーネント（= Atomic Design の Pages）
